...
- 该删除仍只是本地工作区状态；如需远端 GitHub workflow 消失，需要单独提交并推送后再验证远端 workflow 不存在。
- 继续做 Bitget WebSocket 模块边界收敛：`bitget_rs/src/api/websocket.rs` 只保留对外 request/event contract 和 re-export，session/SOCKS5/connect 逻辑拆到 `websocket/connection.rs`，auto reconnect client / manager wrapper / URL fallback / backoff / login ack gate 拆到 `websocket/reconnect.rs`；父文件降到 886 行，仍保留原 `bitget_rs::api::websocket::*` 导出路径。
- 继续做 Binance WebSocket 模块边界收敛：`binance_rs/src/api/websocket/websocket_api.rs` 只保留 URL/session/reconnect/manager/hub 逻辑，`BinanceWebsocketEvent` parser 和用户数据事件 DTO 拆到目录级 `websocket/events.rs`；父文件降到 862 行，`events.rs` 为 490 行，仍保留原 `binance_rs::api::websocket::*` 导出路径。
- 继续做 root adapter 模块边界收敛：`src/adapters/bitget.rs` 只保留 adapter orchestration 和通用 JSON helper，Bitget trade/order/orderbook/candle/fill 映射 helper 拆到 `src/adapters/bitget/trade_mapping.rs`；父文件降到 995 行，新映射模块 322 行，root facade 行为保持不变。
...
Latest iteration: after the fallback/message-timeout slice, started Phase 27 manager/client split by extracting `BitgetAutoReconnectWebsocketClient` from the previous manager state machine. TDD evidence: `auto_reconnect_client_reconnects_and_replays_subscriptions` first failed because the client type did not exist, then `cargo test -p bitget_rs auto_reconnect_client_reconnects_and_replays_subscriptions -- --nocapture` and `cargo test -p bitget_rs websocket_manager_ -- --nocapture && cargo test -p bitget_rs auto_reconnect_client_ -- --nocapture` passed. Full verification run: `cargo fmt && cargo clippy -p bitget_rs --all-targets && cargo test -p bitget_rs && cargo test --workspace` completed successfully. This follow-up iteration mechanically split Bitget WebSocket into `connection.rs` and `reconnect.rs` without changing public imports; verification: `cargo test -p bitget_rs websocket -- --nocapture` before and after the split both passed 15 tests, then `cargo check -p bitget_rs`, `cargo fmt --all -- --check`, `cargo test -p bitget_rs --quiet`, targeted line-limit and `git diff --check` passed. This Binance maintenance iteration mechanically split WebSocket event parsing/DTOs into `binance_rs/src/api/websocket/events.rs` without changing public imports; verification before and after the split: `cargo test -p bnb_rs websocket_event_parser -- --nocapture` passed 2 tests and `cargo test -p bnb_rs --test websocket_api_tests -- --nocapture` passed 10 tests, then `cargo check -p bnb_rs`, targeted line-limit and `git diff --check` passed. This root adapter maintenance iteration mechanically split Bitget trade/order/orderbook/candle/fill mapping into `src/adapters/bitget/trade_mapping.rs`; verification: `cargo test -p crypto_exc_all bitget -- --nocapture`, focused external consumer tests for orderbook/candles, place/cancel, order queries and fills, `cargo check -p crypto_exc_all`, `cargo fmt -- --check`, targeted line-limit and `git diff --check` all passed. No live order or live WebSocket smoke test was executed in this iteration.
