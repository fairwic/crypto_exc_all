...
impl OkxWebsocketManager {
    ...
    async fn execute_subscription(&self, channel: &ChannelType, args: &Args) -> Result<(), Error> {
        match channel {
            ...
            | ChannelType::Books
            | ChannelType::Trades
            | ChannelType::BooksLite
            | ChannelType::Books50L
            | ChannelType::BlockTickers
            | ChannelType::IndexTickers
            | ChannelType::MarkPrice
            | ChannelType::PriceLimit
            | ChannelType::EstimatedPrice
            | ChannelType::FundingRate
            | ChannelType::Status
            | ChannelType::IndexCandle(_)
            | ChannelType::MarkPriceCandle(_) => {
                self.public_client
                    .lock()
                    .await
            ...
            | ChannelType::AdvancedAlgoOrders
            | ChannelType::OrdersAlgo
            | ChannelType::Balance
            | ChannelType::PositionRisk
            | ChannelType::BalanceAndPosition
            | ChannelType::Greeks
            | ChannelType::DepositInfo => {
                if let Some(private_client) = &self.private_client {
                    private_client
                        .lock()
                        .await
                    ...
                    Err(Error::WebSocketError(
                        "Private client not available".to_string(),
                    ))
                }
            }
            // 自定义频道默认使用公共客户端
    ...
    /// 执行取消订阅
    async fn execute_unsubscription(
        &self,
        channel: &ChannelType,
        args: &Args,
    ) -> Result<(), Error> {
