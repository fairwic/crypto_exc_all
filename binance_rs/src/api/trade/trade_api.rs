...
/// Order request for `/fapi/v1/order`.
///
/// Conditional order types are standalone orders; they are not atomically attached to a
/// separate opening order.
...
impl NewOrderRequest {
    ...
    pub fn stop_market(
        ...
        Self::new(symbol, side, "STOP_MARKET").with_stop_price(stop_price)
    }
    ...
    pub fn with_stop_price(mut self, value: impl Into<String>) -> Self {
        self.stop_price = Some(value.into());
        self
    }
    ...
    pub fn to_params(&self) -> Vec<(&'static str, String)> {
        ...
        push_optional(&mut params, "stopPrice", self.stop_price.as_deref());
        ...
        push_optional(
            &mut params,
            "closePosition",
            self.close_position.map(|v| v.to_string()),
        );
        push_optional(&mut params, "workingType", self.working_type.as_deref());
        push_optional(
            &mut params,
            "priceProtect",
            self.price_protect.map(|v| v.to_string()),
        );
        push_optional(
            &mut params,
            "newClientOrderId",
